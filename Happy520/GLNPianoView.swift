...
@IBDesignable class GLNPianoView : UIView {
    ...
    override func layoutSubviews() {
        ...
        blackUpImg = blackKeyImage(CGSize(width: blackKeyWidth, height: blackKeyHeight), keyDown:false)
        blackDoImg = blackKeyImage(CGSize(width: blackKeyWidth, height: blackKeyHeight), keyDown:true)
        whiteUpImg = whiteKeyImage(CGSize(width: 21, height: 21), keyDown:false)
        whiteDoImg = whiteKeyImage(CGSize(width: 21, height: 21), keyDown:true)
        ...
        for i in 0 ..< totalNumKeys {
            if (isWhiteKey(i)) {
                ...
                keyLayers[i] = createCALayer(UIColor.white, aRect:keyRect, white:true)
        ...
        for i in 0 ..< self.totalNumKeys {
            if (isWhiteKey(i)) {
                ...
                keyLayers[i] = createCALayer(UIColor.black, aRect:keyRect, white:false)
                layer.addSublayer(keyLayers[i] as! CALayer)
            }
        }
    }
    ...
    func updateKeyStates() {
        ...
        for i in 0 ..< totalNumKeys {
            if (keyDown[i] as! Bool != currentKeyState[i] as! Bool) {
                ...
                keyDown[i] = NSNumber(value: keyDownState as! Bool)
    ...
    func blackKeyImage(_ aSize:CGSize, keyDown:(Bool)) -> UIImage {
        ...
        let gradient1 = CGGradient(colorsSpace: colorSpace, colors: gradient1Colors as CFArray, locations: gradient1Locations)
        ...
        context?.drawLinearGradient(gradient1!, start: CGPoint(x: roundedRectangleRect.midX, y: roundedRectangleRect.minY),
                                    end: CGPoint(x: roundedRectangleRect.midX, y: roundedRectangleRect.maxY), options: [])
        ...
        context?.drawLinearGradient(gradient1!, start: CGPoint(x: roundedRectangle2Rect.midX, y: roundedRectangle2Rect.maxY),
                                    end: CGPoint(x: roundedRectangle2Rect.midX, y: roundedRectangle2Rect.minY), options: [])
        context?.restoreGState()
    ...
    func whiteKeyImage(_ aSize:CGSize, keyDown:(Bool)) -> UIImage {
        ...
        context.drawRadialGradient(gradient1, startCenter: CGPoint(x: size.width/2.0, y: size.height/2.0), startRadius: size.height * 0.01,
                                    endCenter: CGPoint(x: size.width/2.0, y: size.height/2.0), endRadius: size.height * 0.6, options: [.drawsBeforeStartLocation, .drawsAfterEndLocation])
        context.restoreGState()
